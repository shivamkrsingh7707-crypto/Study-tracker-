...
ONLINE_THRESHOLD_MINUTES = 5  # user considered online if seen in last 5 mins
...
def touch_user(user):
    """Update last_seen to mark user as online. Called on every page load."""
    if user:
...
def get_online_users(limit=20):
    threshold = datetime.utcnow() - timedelta(minutes=ONLINE_THRESHOLD_MINUTES)
    return User.query.filter(User.last_seen >= threshold).order_by(User.last_seen.desc()).limit(limit).all()
  ...
  <div class="w-full max-w-md fade-in">
    ...
    <div class="card-base p-8 relative overflow-hidden" style="box-shadow:0 25px 60px rgba(0,0,0,0.5);">
      ...
      <div class="relative z-10">
        ...
        <p class="text-muted text-sm mb-8">Join {{ total_users }}+ students. Set your details and unlock your personalized dashboard.</p>
        {% if error %}
        ...
        <div class="flex items-center justify-center gap-2 mt-6 text-xs text-muted">
          ...
          <span>{{ online_count }} student{{ 's' if online_count != 1 else '' }} online now</span>
        </div>
      </div>
    </div>
  </div>
...
""" + BASE_STYLE + """
    ...
    <div class="max-w-2xl mx-auto px-4 pt-6 pb-4">
      ...
      <div class="flex items-center justify-between mb-4 fade-in">
        <a href="/chat" class="flex items-center gap-2 px-3 py-1.5 rounded-full" style="background:rgba(0,242,96,0.08);border:1px solid rgba(0,242,96,0.25);">
          <span class="inline-block w-2 h-2 rounded-full online-pulse" style="background:#00F260;"></span>
          <span class="text-xs font-bold" style="color:#00F260;" id="onlineCounter">{{ online_count }}</span>
          <span class="text-xs text-muted">online now</span>
        </a>
        <div class="text-xs text-muted">{{ total_users }} total learners</div>
      </div>
      ...
      <a href="/chat" class="mt-4 fade-in block card-base card-glow" style="animation-delay:0.25s;border-radius:16px;background:linear-gradient(135deg,rgba(0,225,253,0.08),rgba(197,51,255,0.08));border:1px solid rgba(0,225,253,0.15);padding:16px 20px;">
        <div class="flex items-center gap-3">
          ...
          <div class="flex-1">
            <div class="text-sm font-semibold text-white">Join Community Chat</div>
            <div class="text-xs text-muted">{{ online_count }} learners online · Discuss doubts & share tips →</div>
          </div>
        </div>
      </a>
    </div>
  ...
  <script>
    ...
    // Refresh online counter every 30 seconds
    setInterval(async () => {
      try {
        const r = await fetch('/api/online_count');
        ...
        <div class="flex items-center gap-3">
          ...
          <div class="flex-1">
            ...
            <div class="text-sm font-bold text-white">
              {% if exam_plan.days_left < 0 %}
                <span style="color:#FF6B6B;">Exam passed</span>
              {% elif exam_plan.days_left == 0 %}
                <span style="color:#FF6B6B;">Today is exam day! 🎯</span>
              {% else %}
                <span style="color:#FF6B6B;">{{ exam_plan.days_left }} day{{ 's' if exam_plan.days_left != 1 else '' }} left</span> · Study <span style="color:#00E1FD;">{{ exam_plan.per_day_units }} unit{{ 's' if exam_plan.per_day_units != 1 else '' }}/day</span>
              {% endif %}
            </div>
          </div>
...
CHAT_TEMPLATE = """
<!DOCTYPE html><html lang="en"><head>
<title>Community Chat — StudyTracker</title>
""" + BASE_STYLE + """
<script>
...
function renderMessage(m, prepend=false) {
  ...
  container.className = 'flex gap-2 mb-3 msg-in ' + (isMine ? 'flex-row-reverse' : '');
  ...
  // Long-press to reply / click to reply
  bubble.style.cursor = 'pointer';
  bubble.title = 'Tap to reply';
  bubble.addEventListener('click', () => {
    setReply(m.id, m.user_name, m.message);
  });
  ...
  if (prepend) scroll.insertBefore(container, scroll.firstChild);
...
async function sendMessage() {
  ...
  try {
    const resp = await fetch('/api/chat/send', {
      ...
      body: JSON.stringify({room: CURRENT_ROOM, message: text, reply_to_id: replyToId})
    });
    const data = await resp.json();
    if (data.success) {
      cancelReply();
...
async function fetchNewMessages() {
  try {
    const resp = await fetch(`/api/chat/messages?room=${encodeURIComponent(CURRENT_ROOM)}&after=${lastMsgId}`);
    const data = await resp.json();
    if (data.success && data.messages.length > 0) {
      ...
      if (onlineEl && data.online_count !== undefined) onlineEl.textContent = data.online_count;
    }
  } catch(e) {}
}
...
function isNearBottom() {
  const s = document.getElementById('chatScroll');
  return s.scrollHeight - s.scrollTop - s.clientHeight < 100;
}

document.addEventListener('DOMContentLoaded', () => {
  // Initial render of pre-loaded messages
      ...
      <div class="px-4 pt-6 pb-3">
        <div class="flex items-center gap-3 mb-4 fade-in">
          ...
          <div class="flex-1">
            <div class="flex items-center gap-2">
              <span class="text-2xl">💬</span>
              <h1 class="text-xl font-black text-white">Community Chat</h1>
            </div>
            <div class="flex items-center gap-2 mt-0.5">
              <span class="inline-block w-2 h-2 rounded-full online-pulse" style="background:#00F260;"></span>
              <span class="text-xs" style="color:#00F260;"><span id="chatOnlineCount">{{ online_count }}</span> online</span>
              <span class="text-xs text-muted">· {{ total_users }} learners</span>
            </div>
          </div>
        </div>
...
function closeScheduleModal() { document.getElementById('scheduleModal').classList.remove('active'); }
  ...
  <div class="fixed pointer-events-none" style="bottom:-100px;left:-100px;width:500px;height:500px;background:radial-gradient(circle,rgba(0,225,253,0.06) 0%,transparent 70%);filter:blur(80px);z-index:0;"></div>
  ...
  <div class="relative z-10 scroll-content">
    <div class="max-w-2xl mx-auto px-4 pt-8 pb-4">
      ...
      <div class="card-base p-5 mb-4 fade-in" style="animation-delay:0.12s;background:linear-gradient(135deg,rgba(0,242,96,0.05),rgba(0,225,253,0.05));border-color:rgba(0,242,96,0.15);">
        <h2 class="text-sm font-bold text-white mb-3 flex items-center gap-2">🌐 Community Pulse</h2>
        <div class="grid grid-cols-2 gap-3">
          <div class="rounded-xl p-3 text-center" style="background:rgba(0,242,96,0.05);border:1px solid rgba(0,242,96,0.15);">
            <div class="flex items-center justify-center gap-1">
              <span class="inline-block w-2 h-2 rounded-full online-pulse" style="background:#00F260;"></span>
              <div class="text-2xl font-black" style="color:#00F260;">{{ online_count }}</div>
            </div>
      ...
      <div class="space-y-3">
        ...
        <div class="card-base p-4 fade-in" style="animation-delay:{{ 0.18 + loop.index0 * 0.05 }}s;border-radius:14px;">
          ...
          <div class="flex justify-between mt-1.5 text-xs text-muted">
            <span>Each bar = 1 chapter</span>
            <span style="color:#00F260;">■ Done</span>
          </div>
          {% endif %}
        </div>
        {% endfor %}
  ...
  <div class="relative z-10 scroll-content">
    <div class="max-w-2xl mx-auto px-4 pt-8 pb-4">
      ...
      <div class="card-base p-5 fade-in" style="animation-delay:0.2s;border-color:rgba(255,100,100,0.15);">
        ...
        <p class="text-xs text-muted mb-4">This will permanently delete all your data, including chat messages, progress & exam schedules.</p>
...
def setup():
    ...
    if not name or not student_class:
        return render_template_string(
            ...
            online_count=get_online_count(), total_users=get_total_user_count()
        )
    ...
    # Pick a random avatar color based on name length for variety
...
def update_progress():
    ...
    if existing:
        ...
        if not prev and completed:
            delta = 1
        elif prev and not completed:
            delta = -1
    else:
...
def exam_zone():
    ...
    for p in exam_plans:
        if p['status'] in ('upcoming', 'today'):
            nearest_days = p['days_left']
            break
    return render_template_string(
        EXAM_ZONE_TEMPLATE,
        user=user, exam_plans=exam_plans,
...
def chat():
    ...
    online_users = get_online_users(20)
    online_users_data = [{"name": u.name, "student_class": u.student_class, "avatar_color": u.avatar_color or '#00E1FD'} for u in online_users]
...
def api_online_count():
    ...
    if user:
        touch_user(user)
    return jsonify({'online': get_online_count(), 'total': get_total_user_count()})
